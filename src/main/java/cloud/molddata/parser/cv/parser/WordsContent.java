...
public class WordsContent {
    ...
    /*private static final Set<Pattern> fCOMMON_NAME_PARSE = new LinkedHashSet<>();

    static {
        fCOMMON_NAME_PARSE.add(Pattern.compile("([A-Z][a-z]+-[A-Z][a-z]+\\s([A-Z][a-z]+))")); //Ikhe-Abdu Adalahi
        fCOMMON_NAME_PARSE.add(Pattern.compile("([A-Z][a-z]+\\s){2,}")); // Ivan Petrovich and Ivan Petro Borisich
        fCOMMON_NAME_PARSE.add(Pattern.compile("([A-Z][a-z]+\\s[A-Z]\\.((|\\s)[A-Z]\\.|\\s))")); //Kanagava V.B.
    }

    public static Set<Pattern> getfCOMMON_NAME_PARSE() {
        return fCOMMON_NAME_PARSE;
    }

    public static String is_NAME_PARSE(String text){
        String result=null;
        for(Pattern pattern:fCOMMON_NAME_PARSE){
            Matcher name=pattern.matcher(text);
            if(name.find())
                result=name.group();
        }
        return result;
    ...
    public static String is_NAME_PARSE(String text) {
        //Pattern pName3 = Pattern.compile("([A-Z][a-z]+-[A-Z][a-z]+\\s([A-Z][a-z]+)+!\\s)"); //Ikhe-Abdu Adalahi with no s
        //Pattern pName4 = Pattern.compile("(([A-Z][a-z]+\\s){2,})!\\s"); // Ivan Petrovich and Ivan Petro Borisich with now s
    ...
    private static void addWordsFromFileToSet(String srcFile, Set<String> set) {
        //System.out.println("start addWordsFromFileToSet method");
        ArrayList<String> words = readFromFile(srcFile);
        for (String word : words) {
            //System.out.println("addWordsFromFileToSet" + word + " " + srcFile);
            set.add(makeRegular(word));
        }
    }
